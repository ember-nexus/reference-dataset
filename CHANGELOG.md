...
## Unreleased
### Added
- Add relation with a file to scenario `security.filePermission`.

### Changed
- Lint JSON files in parallel through new script `lint-json.sh`, usable both locally and in CI.

### Fixed
- Fix file count and duplicate id check in `update-backup.sh`, add check for files without corresponding element.
